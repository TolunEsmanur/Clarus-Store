        ...
        <div className='gap-x-14 items-center max-w-screen-xl mx-auto px-4 md:flex md:px-8'>
          ...
          <div className={`${show ? "flex flex-col pb-2" : "hidden"} md: flex md:flex-row flex-1 items-center` }>
            <ul className='md:flex md:space-x-6'>
              {navigation.map(item=>(
                <li className="text-gray-700 font-medium flex justify-center" key={item.title} onClick={()=>setShow(false)}>
                <NavLink className={`block hover:bg-main rounded-full py-2 px-4 hover:text-white ${location.pathname===item.path ? "underline scale-110" : ""}`} to={item.path}>{item.title}
               </NavLink>
                </li>
              ))}
            </ul>
